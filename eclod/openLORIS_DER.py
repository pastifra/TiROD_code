...
class DERBufferDataset(Dataset):
    """
    ...
    """
    def __init__(self, dataset_n, TrainTask, buffer_size=250):
        ...
        self.buffer_dataset = []
        for i in self.buffer_indices:
            x = dataset_n[i]
            print(x['img'].shape)
            x = TrainTask._preprocess_batch_input(x)
            img = x['img']
            print(img.shape)
            feat = TrainTask.model.backbone(img)
            fpn_feat = TrainTask.model.fpn(feat)
            z = TrainTask.model.head(fpn_feat)
            x = dataset_n[i]
            x['logits'] = z
            self.buffer_dataset.append(x)
...
for task in range (1, total_tasks+1):
    ...
    if task == 1:
        ...
        buffer_dataset = DERBufferDataset(train_dataset, TrainTask)
        print(buffer_dataset[0])
        print(1/0)
    else:
