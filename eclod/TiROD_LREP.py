...
class ReplayDataLoader:
    def __init__(self, dataset1, dataset2, batch_size, shuffle):
        ...
        self.dataset1_loader = torch.utils.data.DataLoader(
            self.dataset1,
            batch_size= self.batch_size//2,#2
...
class StandardBufferDataset(Dataset):
    ...
    def smart_update_buffer2(self, dataset_np1, task):
        ...
        # Take a random subset of the new task dataset
        new_rand_indices = random.sample(range(len(dataset_np1)), self.buffer_size - len(subset_n))
        subset_np1 = Subset(dataset_np1, new_rand_indices)
...
for task in range (1, 11):
    ...
    if task == 1:
        ...
        buffer_dataset = StandardBufferDataset(train_dataset, buffer_size=150)
    else:
        print("Updating buffer dataset")
        buffer_dataset.smart_update_buffer2(train_dataset, task)
