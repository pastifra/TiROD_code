...
from nanodet.trainer.ewc import EWCTrainingTask
from nanodet.util.ewc import EWCTrainingTask
...
for task in range (1, 11):
    ...
    if task == 1:
        ...
        TrainTask = EWCTrainingTask(cfg, evaluator)
        ...
        TrainTask.compute_fisher_information(train_dataloader)
        TrainTask.store_optimal_params()
        logger.info("Loaded model weight from {}".format(cfg.schedule.load_model if task > 2 else 'baseModels/CUMULtask1/model_last.ckpt'))
    ...
    if task > 1:
        trainer = pl.Trainer(
            ...
            gradient_clip_val=cfg.get("grad_clip", 0.0), #To avoid exploding gradients
            strategy=strategy,
            precision=precision,
        )
        trainer.fit(TrainTask, train_dataloader, val_dataloader, ckpt_path=model_resume_path)
        fisher_inf = TrainTask.fisher_information_list
    if task == 1:
