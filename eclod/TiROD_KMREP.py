...
class ContrastiveBufferDataset(Dataset):
    ...
    def __init__(self, dataset_n, buffer_size=300, device='cuda:2', model_path = '/home/pasti/PycharmProjects/Robot_CLOD/simclr/pth/resnet50_imagenet_bs2k_epochs600.pth.tar'):
    ...
    def get_contr_buffer(self, dataset_n, update_size):
        ...
        for item in dataset_n:
            ...
            inp = self.preprocess(img).unsqueeze(0).to(self.device)
            with torch.no_grad():  # Disable gradient calculation
                encoder_out = self.encoder(inp)[1].detach().cpu()  # 0 -> 2048, 1 -> 128 # Detach and move to CPU
            encoder_outs.append(encoder_out)
