...
from nanodet.trainer.latent_dist_task_runtime import LatentDistTrainingTaskRun
...
for task in range (1, total_tasks+1):
    ...
    if old_val_dataset is not None:
        print('concatenating old task test dataset to new task test dataset')
        val_dataset = CustomConcatDataset([val_dataset, old_val_dataset])
    """
    ...
    if task == 1:
        ...
        TrainTask = LatentDistTrainingTask(cfg, 1.0, 1.0, evaluator)
    ...
    if(task > 1):
        ...
        torch.save({'state_dict': new_state_dict}, 'models/' + args.Loris_task + args.factor + 'task' + str(task) + '/model_last.ckpt')
    ####END OF TRAINING
